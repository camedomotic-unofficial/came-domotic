...
async def test_load_shedding_switch_states(hass):
    ...
    oven = hass.states.get("switch.oven_load_shedding")
    assert oven is not None
    assert oven.state == "on"

    heat_pump = hass.states.get("switch.heat_pump_load_shedding")
...
async def test_load_shedding_switch_turn_off(hass):
    ...
    with patch.object(
        ...
        await hass.services.async_call(
            "switch",
            "turn_off",
            {"entity_id": "switch.oven_load_shedding"},
            blocking=True,
        )
    ...
    state = hass.states.get("switch.oven_load_shedding")
    assert state.state == "off"
...
async def test_load_shedding_switch_turn_on(hass):
    ...
    with patch.object(
        ...
        await hass.services.async_call(
            "switch",
            "turn_on",
            {"entity_id": "switch.heat_pump_load_shedding"},
            blocking=True,
        )
    ...
    state = hass.states.get("switch.heat_pump_load_shedding")
    assert state.state == "on"
